...
*   **Mapeamento Lexical Computacional (Regex):**
    ```python
    # Dicionário de Modulação Semântica por Classe
...
for i, line in enumerate(lines):
    if line.startswith("**A. Geração Estruturada (Sinônimos Simples)**"):
        start_idx = i
        break
...
for i in range(start_idx, len(lines)):
    if line.startswith("Todos os dados gerados foram concatenados à base de treino"):
        end_idx = i
        break
    # Or match based on text that comes right after block C
    if "Todos os dados gerados foram concatenados à base de treino" in lines[i]:
        end_idx = i
        break
